...
export class Md2Clock {
  ...
  get hand(): any {
    ...
    if (this._view) {
      let inner = this._hour >= 0 && this._hour < 12;
  ...
  private twelveHour(hour) {
    const newHour = (hour % 12 === 0) ? 12 : hour % 12;
    const suffix = hour >= 12 ? 'pm' : 'am';
    return newHour === 12 ? newHour + suffix : newHour;
  }
  ...
  private setTime(event: any) {
    ...
    let unit = Math.PI / (this._view ? 6 : (this.interval ? (30 / this.interval) : 30));
